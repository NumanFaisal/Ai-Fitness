...
export interface DynamicDietPlanResult {
  ...
  generatedBy: string;
  source: "GROQ" | "GEMINI" | "ALGORITHMIC_FOOD_DATABASE";
  macroValidation: {
    caloriesWithinTolerance: boolean;
    proteinWithinTolerance: boolean;
    zeroAllergensVerified: boolean;
  };
}
...
export class NutritionPlanner {
   ...
   */
  async assembleDynamicPlan(context: PersonalizationContext): Promise<DynamicDietPlanResult> {
    const allRecipes = await recipeService.getAllRecipes();
    const { nutrition, goal, profile } = context;
    const targetCalories = nutrition.authoritativeCalorieTarget;
    const targetProtein = (nutrition as any).authoritativeProteinTargetG ?? (nutrition as any).authoritativeProteinTarget ?? 150;

    // Macro slot allocation
    const slotTargets = [
      { slot: "BREAKFAST" as const, calRatio: 0.25, protRatio: 0.25 },
      { slot: "LUNCH" as const, calRatio: 0.35, protRatio: 0.35 },
      { slot: "SNACK" as const, calRatio: 0.15, protRatio: 0.15 },
      { slot: "DINNER" as const, calRatio: 0.25, protRatio: 0.25 },
    ];

    const plannedMeals: PlannedMealItem[] = [];
    ...
    for (const { slot, calRatio, protRatio } of slotTargets) {
      ...
      let selectedRecipe = safeCandidates.find((r) => r.budgetTiers.includes(nutrition.budgetTier as any));
      ...
      if (!selectedRecipe) {
        const slotFallback = FOOD_RECIPES_CATALOG.filter((r) => r.mealSlot === slot);
      ...
      const safeIngredients = this.sanitizeIngredients(
        scaledIngredients,
        nutrition.allergies,
    ...
    for (const meal of plannedMeals) {
      for (const ing of meal.ingredients) {
        const check = foodService.hasAllergenConflict(ing, [], nutrition.allergies);
  ...
  private sanitizeIngredients(ingredients: string[], allergies: string[], dislikes: string[]): string[] {
    ...
    return ingredients.map((ing) => {
      ...
      if (rawAllergies.some((a) => a.includes("dairy") || a.includes("milk") || a.includes("cheese")) &&
        ...
        if (lower.includes("whey")) return "30g Plant-Based Pea/Rice Protein Isolate (Dairy-Free)";
        if (lower.includes("yogurt") || lower.includes("curd")) return "200g Dairy-Free Coconut/Almond Alternative Yogurt";
      ...
      if (rawAllergies.some((a) => a.includes("gluten") || a.includes("wheat")) && (lower.includes("bread") || lower.includes("oats") || lower.includes("wheat"))) {
        if (lower.includes("bread")) return "2 Slices Certified Gluten-Free Seed Bread";
