...
class ExerciseService {
    ...
    async getAllExercises() {
        try {
            const dbExercises = await db_1.prisma.exercise.findMany({
                include: { media: true },
            });
            if (dbExercises && dbExercises.length > 0) {
                return dbExercises.map((e) => ({
                    ...
                    media: {
                        mediaType: e.media?.mediaType || "gif",
        ...
        return exerciseCatalog_1.EXERCISE_CATALOG;
    }
    /**
     * Retrieves compatible exercises matching user's equipment and injury constraints
     */
    getCompatibleExercises(exercises, equipmentAvailable, injuries, isGym, prohibitedExercises = []) {
        ...
        return exercises.filter((ex) => {
            if (prohibitedSet.has(ex.slug.toLowerCase()))
                return false;
            // Injury conflict check
            const conflictsWithInjury = injuries.some((inj) => {
                const lower = inj.toLowerCase();
                if (ex.injuryConsiderations?.some((c) => c.toLowerCase().includes(lower) || lower.includes(c.toLowerCase())))
                    return true;
                if (ex.contraindications?.some((c) => c.toLowerCase().includes(lower) || lower.includes(c.toLowerCase())))
                    return true;
    ...
    getExerciseBySlug(slug) {
        ...
        return exerciseCatalog_1.EXERCISE_CATALOG.find((e) => e.slug.toLowerCase() === lower || e.name.toLowerCase() === lower);
    }
    /**
     * Retrieves safe alternatives for a specific exercise
     */
    getAlternatives(slug, options) {
        let pool = exerciseCatalog_1.EXERCISE_CATALOG;
        if (Array.isArray(options)) {
            pool = options;
        }
        else if (options && typeof options === "object") {
            pool = this.getCompatibleExercises(exerciseCatalog_1.EXERCISE_CATALOG, options.equipmentAvailable || ["barbell", "dumbbell", "bodyweight"], options.injuries || [], false);
        }
        const current = exerciseCatalog_1.EXERCISE_CATALOG.find((e) => e.slug === slug);
        if (!current)
        ...
        return pool.filter((e) => e.slug !== slug &&
