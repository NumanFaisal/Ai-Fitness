import { NutritionPlanOutput } from "./nutritionEngine";
...
export interface RecalibrationInput {
  ...
  currentPlan: NutritionPlanOutput;
  userProfile: {
    heightCm: number;
    age: number;
...
export interface RecalibrationResult {
  ...
  rateDivergenceKgPerWeek: number;
  adaptationReason: string;
  updatedPlan: NutritionPlanOutput;
}

/**
 * Calculates rolling average weight and weekly change rate using linear trend over logs.
 */
export function calculateWeightTrend(logs: WeightLogEntry[]): {
  averageWeightKg: number;
  weeklyRateKg: number;
  daysSpan: number;
} {
  if (logs.length === 0) return { averageWeightKg: 75, weeklyRateKg: 0, daysSpan: 0 };
  if (logs.length === 1) return { averageWeightKg: logs[0].weightKg, weeklyRateKg: 0, daysSpan: 0 };
 ...
 */
export function recalibrateTDEEAndNutrition(input: RecalibrationInput): RecalibrationResult {
  ...
  let expectedRate = 0;
  if (goal === "FAT_LOSS") expectedRate = -0.5;
  else if (goal === "MUSCLE_GAIN") expectedRate = 0.25;
  else if (goal === "RECOMPOSITION" || goal === "MAINTENANCE") expectedRate = 0.0;
  else expectedRate = -0.25;
  ...
  const updatedPlan: NutritionPlanOutput = {
    ...currentPlan,
