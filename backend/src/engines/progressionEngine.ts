...
export function evaluateExerciseProgression(
  ...
  const cleanSetsAtCeiling = recentLogs.filter(
    (s) => s.reps >= prescription.repRangeHigh && (s.rpe === undefined || s.rpe <= 8.5)
  ).length;

  const averageRPE =
    recentLogs.reduce((acc, s) => acc + (s.rpe || 8.0), 0) / Math.max(1, recentLogs.length);
