...
exports.generateAIPlanDetails = generateAIPlanDetails;
exports.cleanCoachText = cleanCoachText;
exports.chatWithCoach = chatWithCoach;
exports.askAICoach = askAICoach;
...
async function callLLM(systemPrompt, userPrompt, jsonFormat = false) {
 ...
 */
async function generateAIPlanDetails(userContext) {
...
Write the coaching narrative and meal plan for this exact profile and image analysis — do not generalize to a default user.`;
    ...
    const dynamicPlan = await nutritionPlanner_1.nutritionPlanner.assembleDynamicPlan({
        userId: "session_user",
        profile: {
            name: userContext.name,
            age: userContext.age,
            sex: userContext.sex || "MALE",
            heightCm: 175,
            weightKg: 75,
        },
        goal: {
            type: userContext.goal || "GENERAL_FITNESS",
            isPrimary: true,
        },
        training: {
            experienceLevel: userContext.experienceLevel || "BEGINNER",
            trainingEnvironment: userContext.trainingEnvironment || "GYM",
        ...
        nutrition: {
            ...
            cuisinePreferences: [],
            budgetTier: userContext.budgetTier || "MEDIUM",
...
async function chatWithCoach(userMessage, userContext) {
    const lower = userMessage.toLowerCase();
    // Safety Gate: Medical & Severe Pain Escalation Guardrail
    if (lower.includes("hurt") ||
        lower.includes("pain") ||
 ...
 */
async function askAICoach(userContext, userMessage, _history = []) {
