...
async function persistPlanToSupabase(userId, email, workout, nutrition) {
    try {
        const dbUserId = await (0, db_1.ensureUserExists)(userId, email);
        if (workout?.days && workout.days.length > 0) {
            await db_1.prisma.workoutPlan.deleteMany({ where: { userId: dbUserId } }).catch(() => { });
            await db_1.prisma.workoutPlan.create({
                data: {
                    userId: dbUserId,
                    status: "ACTIVE",
                    startDate: new Date(),
        ...
        if (nutrition?.calorieTarget?.value) {
            await db_1.prisma.nutritionPlan.deleteMany({ where: { userId: dbUserId } }).catch(() => { });
            await db_1.prisma.nutritionPlan.create({
                data: {
                    userId: dbUserId,
                    status: "ACTIVE",
