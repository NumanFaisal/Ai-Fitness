...
exports.calculatePhysiologicalTargetWeight = calculatePhysiologicalTargetWeight;
exports.analyzeTargetPhysique = analyzeTargetPhysique;
exports.analyzeUserBodyPhoto = analyzeUserBodyPhoto;
 ...
 */
function calculatePhysiologicalTargetWeight(heightCm, currentWeightKg, sex, targetBodyFatPct, goal) {
 ...
 */
async function analyzeTargetPhysique(input) {
 ...
 */
async function analyzeUserBodyPhoto(input) {
...
Return ONLY a valid JSON object matching this schema without code fences or extra text:
{
  ...
  "confidenceScore": 0.88
                    ...
                    if (imagePart) {
                        ...
                        if (res.ok) {
                            ...
                            if (text) {
                                ...
                                const cat = parsed.bodyFatCategory ||
                                    (bf < (isMale ? 12 : 20)
                                        ? "LEAN"
                                        : bf < (isMale ? 18 : 26)
                                            ? "ATHLETIC"
    ...
    const bodyFatCategory = clampedBf < (isMale ? 13 : 20)
        ? "LEAN"
        : clampedBf < (isMale ? 18 : 26)
            ...
            : clampedBf < (isMale ? 25 : 32)
                ? "MODERATE"
                : "HIGH";
    const somatotype = bmi < 21.0 ? "ECTOMORPH" : bmi > 27.5 ? "ENDOMORPH" : "MESOMORPH";
    let visualStrengths;
    let developmentPriorityMuscles;
